...
func TestHetzner(t *testing.T) {
	...
	t.Run("basic container execution", func(t *testing.T) {
		testHetznerBasicExecution(t, token, testLabel)
	})

	t.Run("with auto size", func(t *testing.T) {
		testHetznerAutoSize(t, token, testLabel)
	})

	t.Run("reuse_worker parks and reclaims machine", func(t *testing.T) {
		testHetznerReuseWorker(t, token, testLabel)
	})

	t.Run("max_workers limits concurrent machines", func(t *testing.T) {
		testHetznerMaxWorkers(t, token, testLabel)
	})
}

func testHetznerBasicExecution(t *testing.T, token, testLabel string) {
	assert := NewGomegaWithT(t)

	namespace := "test-" + gonanoid.Must()
	client, err := hetzner.New(context.Background(), hetzner.Config{
...
func testHetznerAutoSize(t *testing.T, token, testLabel string) {
	assert := NewGomegaWithT(t)
...
func testHetznerReuseWorker(t *testing.T, token, testLabel string) {
	assert := NewGomegaWithT(t)
	...
	client1, err := hetzner.New(context.Background(), cfg, slog.Default())
	assert.Expect(err).NotTo(HaveOccurred())
	...
	client2, err := hetzner.New(context.Background(), cfg, slog.Default())
	assert.Expect(err).NotTo(HaveOccurred())
...
func testHetznerMaxWorkers(t *testing.T, token, testLabel string) {
	assert := NewGomegaWithT(t)
