...
// Cleanup is a no-op for QEMU containers — cleanup happens at driver Close.
func (c *Container) Cleanup(_ context.Context) error {
	return nil
}

// pollStatus queries QGA for process status and caches the result.
func (c *Container) pollStatus() error {
	...
	result, err := c.qga.ExecStatus(c.pid)
	if err != nil {
